...
def _check_values_within_bounds(ds, var_name, check_id, severity):
    """
    Internal helper to check if values lie within their bounds.
    """
...
def check_lat_values_within_bounds(ds, severity=BaseCheck.MEDIUM):
    """Verify that each lat value lies within the corresponding lat_bnds range."""
...
def check_lon_values_within_bounds(ds, severity=BaseCheck.MEDIUM):
    """Verify that each lon value lies within the corresponding lon_bnds range."""
