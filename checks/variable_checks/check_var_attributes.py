...
def _check_attr_exists(ds, var_name, attr_name, check_id, severity):
    """Check that an attribute exists on a variable."""
...
def _check_attr_type(ds, var_name, attr_name, expected_type, check_id, severity):
    """Check that an attribute has the expected type."""
...
def _check_attr_utf8(ds, var_name, attr_name, check_id, severity):
    """Check that a string attribute is valid UTF-8."""
...
def _check_attr_value(ds, var_name, attr_name, expected_value, check_id, severity):
    """Check that an attribute has the expected value."""
    ...
    try:
        ...
        if str(attr_val).strip() == str(expected_value).strip():
            ctx.add_pass()
        else:
            ctx.add_failure(
...
def check_height_long_name_value(ds, severity=BaseCheck.MEDIUM):
    return _check_attr_value(ds, "height", "long_name", "height", "V015", severity)
...
def check_lat_long_name_value(ds, severity=BaseCheck.MEDIUM):
    return _check_attr_value(ds, "lat", "long_name", "latitude", "V056", severity)
...
def check_lon_long_name_value(ds, severity=BaseCheck.MEDIUM):
    return _check_attr_value(ds, "lon", "long_name", "longitude", "V094", severity)
...
def check_i_long_name_value(ds, severity=BaseCheck.MEDIUM):
    return _check_attr_value(ds, "i", "long_name", "cell index", "V235", severity)
...
def check_j_long_name_value(ds, severity=BaseCheck.MEDIUM):
    return _check_attr_value(ds, "j", "long_name", "cell index", "V243", severity)
